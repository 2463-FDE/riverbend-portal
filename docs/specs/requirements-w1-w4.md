...
| `RVB-X-08` | **Engineering policy:** PHI never enters a prompt, a trace, a log body, or a vector index without an explicit, tested gate. This is our chosen control, not a verbatim regulatory mandate — 164.502(b) (minimum necessary) and 164.502(e) (business-associate assurances) are what *motivate* it. Stating it the other way round would be the same self-assertion we criticise in the README. | motivated by 164.502(b), 164.502(e) |
| `RVB-X-09` | **Bedrock effective data-retention mode must be `none`**, verified at startup, for every PHI-adjacent path. Any model whose `allowed_modes` excludes `none` is disqualified regardless of capability — selecting one shares prompts and completions with the provider for up to 30 days, which *is* debt D13. Preflight asserts mode **and** model `allowed_modes`, and refuses to serve otherwise. Org-wide SCP denies setting any other mode. | ADR 0004 §1a; 164.502(e) |
...
| `RVB-W1-15` | T | **End-to-end surface test.** Log in → `POST /ai/summary` through the gateway with real intake-instruction text → a grounded summary comes back. Against the stub model, zero spend. | The feature the client clicks is proven, not only its 401. An endpoint that rejected everything would have passed the old criteria. |
| `RVB-W1-16` | C | **Retention preflight** (`RVB-X-09`) implemented and fail-closed. | Test: a mocked effective mode of `default` or `provider_data_share` causes the service to refuse to serve, with a clear error. |
...
| `RVB-W2-10` | C | **The index is a PHI store and is governed as one.** Two ingest paths: `ingest_document` (clinic knowledge → shared collection, lenient scrub that preserves dates/clinic contact) and `ingest_record` (patient corpus → **separate Chroma collection**, direct identifiers stripped where not needed for retrieval, queries **patient-scoped**, access-controlled and audited like the database). We reject scrubbing patient records aggressively — retrieval over patient history *is* the client's ask and a scrubbed corpus cannot serve it. The honest position is that the index holds PHI, not that it can be scrubbed into not holding PHI. | Test: an `ingest_document` scrub applied to a patient record is rejected by type; a query as patient A cannot retrieve patient B's chunks; ingest requires a privileged capability, unprivileged session gets 403. |
| `RVB-W2-13` | C | The `knowledge_admin` capability has a **concrete mechanism**, since the brownfield has only a `staff` role: env-driven username allowlist **plus** role allowlist, enforced at the gateway, surfaced via `/me` so the portal does not guess policy client-side. Explicitly an interim control until the W9 RBAC split. | Test: allowlisted user 200, non-allowlisted 403, `/me` reports the capability. |
| `RVB-W2-14` | T | **End-to-end surface test.** Log in → seed corpus → `POST /ai/knowledge/query` with a chart-shaped clinical question → cited results return. Stub model, zero spend. *Scope note:* this proves the retrieval endpoint answers; wiring retrieval into the chart-open lifecycle is a UI integration outside the packet's sizing and is roadmapped, not silently dropped. | |
...
| `RVB-W3-14` | T | **End-to-end surface test.** Log in → staff chat turn through the gateway → the agent invokes `check_eligibility` and returns a status. Stub model, faked payer, zero spend. | |
...
| `RVB-W4-03` | C | **Authorization check at the graph boundary** — deterministic, executed *before* any retriever runs, on every branch. Not an LLM judgement, not a post-filter on assembled output. **Depends on `RVB-W4-14`:** the current session carries only `username` and `role`, so there is no patient identity to authorize against until the binding exists. | Test: a session **bound to** patient 1042 requesting 1043 is denied with **zero** record rows loaded. |
...
| `RVB-W4-07` | T | **Regression test asserting the HAR walk is now denied** on the *existing* `GET /patients/{id}/records` route. The pre-fix reproduction is preserved as **evidence in the findings doc**, not as a test. A permanently-failing test either breaks CI or, if it passes, enshrines the vulnerability. | Test fails if the gateway check is removed. |
| `RVB-W4-14` | C | **Session → patient identity binding** (ADR 0011). Nullable `users.patient_id` FK with a unique partial index; `create_session` carries it; `/me` exposes it; **server-derived, never client-supplied**. Two principal kinds: patient (own id + `SAME_AS` fragments) and staff (patient in context, coarse until W9's RBAC split — stated, not oversold). | Test: a client-supplied `patient_id` is ignored; a patient principal resolves to own id plus linked fragments. |
| `RVB-W4-15` | C | **The existing route is fixed, not just the new one.** `GET /patients/{id}` and `GET /patients/{id}/records` enforce the scope at the gateway before proxying. Out-of-scope returns **404, not 403** — a 403 on valid ids and 404 on invalid ones is an enumeration oracle. | Test: both routes deny cross-patient access; both return identical responses for a valid-but-unauthorized id and a nonexistent id. |
| `RVB-W4-16` | T | **End-to-end surface test.** A patient logs in and sees their own assembled view; the same session cannot see another patient's. Stub model, zero spend. | |
...
| D11 IDOR on record reads | W4 | **W4 — the existing gateway route is fixed** (ADR 0011 identity binding + scope check on `GET /patients/{id}` and `/records`). The new assembler shares the same gate. | Staff-side minimum-necessary stays coarse → W9 |
| D13 PHI to LLM vendor, no BAA | W1 (avoided by construction) | instructions-only contract **+ `RVB-X-09` retention mode `none`** — a `provider_data_share` model would recreate D13 exactly, so model choice is a compliance control | W8 (Safe-Harbor scrub) |
